...
//this version of the agent neatly separates the initialization phase and the functionality available through RPCs
// (at the cost of having to type "this" rather often)
...
gameOfLifeAgent.init = function() {
  ...
  var notifications = [];
  var result = [];
  var maxtimesteps = this.options.maxCycles;

  var gr = this.options.width;
...
gameOfLifeAgent.broadcast = function(curtimestep, curliving) {
  //console.log("sending sometin:" + curtimestep);
  for (var i = 0; i < this.neighbours.length; i++) {
    this.send("Agent_" +this.neighbours[i],
